...
import { Repository } from '../../common/classes/Repository.ts';
...
import { TenantRepository } from '../../tenant/repository/TenantRepository.ts';
import { AggregatedTenantModel } from '../../tenant/models/TenantModel.ts';


export class UserController extends Controller<UserRepository> {
  constructor(db: Db) {
  ...
  get = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (!isEmpty(aggregate)) {
        const parsedAggregate = parseArgs(aggregate as string, res);
        ...
        const parsedOpts = parseArgs(opts as string, res);
        const parsedFilter = parseArgs(filter as string, res);
