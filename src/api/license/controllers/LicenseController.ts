...
import { LicenseRepository } from '../repository/LicenseRepository.ts';
import { parseArgs } from '../../../utils/utils.ts';


export class LicenseController extends Controller<LicenseRepository> {
  constructor(db: Db) {
    const repository = LicenseRepository.getInstance<LicenseRepository>(COLLECTIONS.LICENSES, db);
    super(repository);
  }
  ...
  get = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const entities = await this.repository.get(filter as Filter<LicenseModel>, opts as FindOptions);
